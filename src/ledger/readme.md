# What is a ledger?
A ledger represents the state of the Fonero universe at a given point in time.
...
[`src/xdr/Fonero-ledger.x`](../xdr/Fonero-ledger.x)
...
foneros present at a given time.
...
During consensus, nodes work together to decide on the value of FoneroValue.
FoneroValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'FoneroValue' should be able to transition to the same new ledger.
...
[`src/xdr/Fonero-ledger-entries.x`](../xdr/Fonero-ledger-entries.x)

### AccountEntry
This entry represents an account. In Fonero, everything is centered around
...
Offers are a way to automate simple trading inside the Fonero network.
