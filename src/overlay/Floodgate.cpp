...
namespace fonero
{
Floodgate::FloodRecord::FloodRecord(FoneroMessage const& msg, uint32_t ledger,
                                    Peer::pointer peer)
...
bool
Floodgate::addRecord(FoneroMessage const& msg, Peer::pointer peer)
{
    if (mShuttingDown)
    {
        return false;
...
void
Floodgate::broadcast(FoneroMessage const& msg, bool force)
{
    if (mShuttingDown)
    {
        return;
