...
static std::string
msgSummary(FoneroMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(FoneroMessage const& msg)
{
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            << "("
...
void
Peer::recvMessage(FoneroMessage const& foneroMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(foneroMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (foneroMsg.type() != HELLO) &&
        (foneroMsg.type() != AUTH) && (foneroMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay")
            << "recv: " << foneroMsg.type() << " before completed handshake";
        mDropInRecvMessageUnauthMeter.Mark();
        drop();
        return;
    }

    assert(isAuthenticated() || foneroMsg.type() == HELLO ||
           foneroMsg.type() == AUTH || foneroMsg.type() == ERROR_MSG);

    switch (foneroMsg.type())
    {
    case ERROR_MSG:
    {
    ...
    case HELLO:
    {
        ...
        this->recvHello(foneroMsg.hello());
    }
    break;
...
void
Peer::recvDontHave(FoneroMessage const& msg)
{
...
void
Peer::recvGetTxSet(FoneroMessage const& msg)
{
    auto self = shared_from_this();
...
void
Peer::recvTxSet(FoneroMessage const& msg)
{
...
void
Peer::recvTransaction(FoneroMessage const& msg)
{
...
void
Peer::recvGetSCPQuorumSet(FoneroMessage const& msg)
{
...
void
Peer::recvSCPQuorumSet(FoneroMessage const& msg)
{
...
void
Peer::recvSCPMessage(FoneroMessage const& msg)
{
...
void
Peer::recvGetSCPState(FoneroMessage const& msg)
{
...
void
Peer::recvError(FoneroMessage const& msg)
{
...
void
Peer::recvAuth(FoneroMessage const& msg)
{
    if (mState != GOT_HELLO)
    {
...
void
Peer::recvGetPeers(FoneroMessage const& msg)
{
    sendPeers();
}

void
Peer::recvPeers(FoneroMessage const& msg)
{
