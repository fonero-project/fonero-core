...
class HerderSCPDriver : public SCPDriver
{
  public:
    struct ConsensusData
    {
        ...
        FoneroValue mConsensusValue;
        ConsensusData(uint64_t index, FoneroValue const& b)
    ...
    void restoreSCPState(uint64_t index, FoneroValue const& value);
    ...
    void nominate(uint64_t slotIndex, FoneroValue const& value,
                  TxSetFramePtr proposedSet, FoneroValue const& previousValue);
  ...
  private:
    ...
    validateValueHelper(uint64_t slotIndex, FoneroValue const& sv) const;
