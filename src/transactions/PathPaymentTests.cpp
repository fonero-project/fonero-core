...
assetToString(const Asset& asset)
{
    ...
    switch (asset.type())
    {
    case fonero::ASSET_TYPE_NATIVE:
        r = std::string{"FNO"};
        break;
    case fonero::ASSET_TYPE_CREDIT_ALPHANUM4:
        assetCodeToStr(asset.alphaNum4().assetCode, r);
        break;
    case fonero::ASSET_TYPE_CREDIT_ALPHANUM12:
...
TEST_CASE("pathpayment", "[tx][pathpayment]")
{
    ...
    SECTION("path payment destination amount 0")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance1 - 2 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{fno, minBalance}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("dest amount too big for FNO")
    {
        auto a = root.create("a", minBalance1);
        for_all_versions(*app, [&] {
            REQUIRE_THROWS_AS(root.pay(a, fno, 20, fno,
    ...
    SECTION("path payment FNO with not enough funds")
    {
        auto market = TestMarket{*app};
        // see https://github.com/fonero-project/fonero-core/pull/1239
        auto minimumAccount =
            root.create("minimum-account", minBalanceNoTx + 2 * txfee + 20);
        for_all_versions(*app, [&] {
            REQUIRE_THROWS_AS(
                minimumAccount.pay(root, fno, txfee + 21, fno, txfee + 21, {}),
                ex_PATH_PAYMENT_UNDERFUNDED);
            // clang-format off
            market.requireBalances(
                {{minimumAccount, {{fno, minBalanceNoTx + txfee + 20}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment asset with not enough funds")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{minimumAccount, {{fno, minBalance1 - 2 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
            ...
            // clang-format off
            market.requireBalances(
                {{minimumAccount, {{fno, minBalance1 - 3 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment source does not have trustline")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{noSourceTrust, {{fno, minBalance - txfee}, {idr, 0}, {usd, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
            ...
            // clang-format off
            market.requireBalances(
                {{noSourceTrust, {{fno, minBalance - 2 * txfee}, {idr, 0}, {usd, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment source is not authorized")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{noAuthorizedSourceTrust, {{fno, minBalance1 - 2 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
            ...
            // clang-format off
            market.requireBalances(
                {{noAuthorizedSourceTrust, {{fno, minBalance1 - 3 * txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
            ...
            "paths")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance1 - 2 * txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment destination does not have trustline")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance1 - txfee}, {idr, 10}, {usd, 0}}},
                 {noDestinationTrust, {{fno, minBalance}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
    ...
    SECTION("path payment destination is not authorized")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance1 - txfee}, {idr, 10}, {usd, 0}}},
                 {noAuthorizedDestinationTrust, {{fno, minBalance1 - txfee}, {idr, 0}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(
    ...
    SECTION("path payment destination line full")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance1 - txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {idr, 10}, {usd, 0}}}});
            // clang-format on
    ...
    SECTION("path payment destination line overflow")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance1 - txfee}, {idr, 10}, {usd, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {idr, std::numeric_limits<int64_t>::max() - 10}, {usd, 0}}}});
            // clang-format on
            REQUIRE_THROWS_AS(source.pay(destination, idr, 11, idr, 11, {}),
                              ex_PATH_PAYMENT_LINE_FULL);
            // clang-format off
    ...
    SECTION("path payment not enough offers for first exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance1 - 2 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment crosses own offer for first exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance3 - 4 * txfee}, {cur1, 10}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
    ...
    SECTION("path payment crosses own offer for middle exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 5 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
    ...
    SECTION("path payment crosses own offer for last exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 5 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
            ...
            "first exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
            ...
            "middle exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            ...
            "last exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
    ...
    SECTION("path payment over send max FNO")
    {
        auto market = TestMarket{*app};
        auto source = root.create("source", minBalance);
        auto destination = root.create("destination", minBalance);
        for_all_versions(*app, [&] {
            REQUIRE_THROWS_AS(source.pay(destination, fno, 10, fno, 11, {}),
                              ex_PATH_PAYMENT_OVER_SENDMAX);
            market.requireBalances(
                {{source, {{fno, minBalance - txfee}, {idr, 0}, {usd, 0}}},
                 {destination, {{fno, minBalance}, {idr, 0}, {usd, 0}}}});
        });
    }
    ...
    SECTION("path payment over send max with real path")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
        });
    ...
    SECTION("path payment to self FNO")
    {
        auto market = TestMarket{*app};
        auto account = root.create("account", minBalance + txfee + 20);

        for_versions_to(7, *app, [&] {
            auto offers = account.pay(account, fno, 20, fno, 20, {});
            auto expected = std::vector<ClaimOfferAtom>{};
            REQUIRE(offers.success().offers == expected);
            market.requireBalances({{account, {{fno, minBalance}}}});
        });

        for_versions_from(8, *app, [&] {
            account.pay(account, fno, 20, fno, 20, {});
            market.requireBalances({{account, {{fno, minBalance + 20}}}});
        });
    }
    ...
    SECTION("path payment crosses destination offer for first exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance1 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{fno, minBalance4 - 4 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance1 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 0}}},
                 {destination, {{fno, minBalance4 - 4 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance1 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 10}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {destination, {{fno, minBalance4 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 10}, {cur4, 10}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment uses whole best offer for first exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{fno, minBalance3 - 3 * txfee}, {cur1, 20}, {cur2, 30}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{fno, minBalance3 - 3 * txfee}, {cur1, 60}, {cur2, 10}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment uses whole best offer for second exchange")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 30}, {cur3, 5}, {cur4, 0}}},
                 {mm23a, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 30}, {cur3, 5}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        ...
        for_all_versions(*app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 4}, {cur4, 8}}},
                 {mm34b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 16}, {cur4, 2}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment reaches limit for offer for first exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{fno, minBalance3 - 4 * txfee}, {cur1, 4}, {cur2, 38}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{fno, minBalance3 - 3 * txfee}, {cur1, 76}, {cur2, 2}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
    ...
    SECTION("path payment reaches limit for offer for second exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0},  {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 4}, {cur3, 18}, {cur4, 0}}},
                 {mm23b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 36}, {cur3, 2}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 2}, {cur4, 9}}},
                 {mm34b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 18}, {cur4, 1}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    ...
    SECTION("path payment missing trust line for offer for first exchange")
    {
        ...
        SECTION("missing selling line")
        {
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12a, {{fno, minBalance3 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12b, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
        ...
        SECTION("missing buying line")
        {
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm12b, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
        ...
        SECTION("missing selling line")
        {
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23a, {{fno, minBalance3 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            });
        ...
        SECTION("missing buying line")
        {
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {mm23b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            ...
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34a, {{fno, minBalance3 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm34b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            });
            ...
            for_versions_to(9, *app, [&] {
                ...
                market.requireBalances(
                    {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                     {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                     {mm34a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                     {mm34b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                     {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
                // clang-format on
            });
            ...
            "first exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            ...
            "second exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm34b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
            ...
            "first exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{fno, minBalance3 - 3 * txfee}, {cur1, 200}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            ...
            "second exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 200}, {cur3, 20}, {cur4, 0}}},
                 {mm23b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 200}, {cur4, 10}}},
                 {mm34b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
            ...
            "first exchange")
    {
        ...
        for_versions_to(2, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 1}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
        ...
        for_versions(3, 9, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 1}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{fno, minBalance3 - 4 * txfee}, {cur1, 1}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{fno, minBalance3 - 3 * txfee}, {cur1, 78}, {cur2, 1}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
            ...
            "second exchange")
    {
        ...
        for_versions_to(2, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 1}, {cur4, 0}}},
                 {mm23b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        ...
        for_versions(3, 9, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 2}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 78}, {cur2, 1}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 1}, {cur3, 0}, {cur4, 0}}},
                 {mm23b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 38}, {cur3, 1}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
            ...
            "exchange")
    {
        ...
        for_versions_to(2, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 1}}},
                 {mm34b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
        for_versions(3, 9, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 4}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 76}, {cur2, 2}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 38}, {cur3, 1}, {cur4, 0}}},
                 {mm34a, {{fno, minBalance3 - 4 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 1}, {cur4, 0}}},
                 {mm34b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 18}, {cur4, 1}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
            ...
            "first exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12a, {{fno, minBalance3 - 3 * txfee}, {cur1, 199}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm12b, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            ...
            "second exchange")
    {
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23a, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 199}, {cur3, 20}, {cur4, 0}}},
                 {mm23b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        ...
        for_versions_to(9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 80}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 40}, {cur3, 0}, {cur4, 0}}},
                 {mm34a, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 199}, {cur4, 10}}},
                 {mm34b, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 20}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    ...
    SECTION("path payment takes all offers, multiple offers per exchange")
    {
        ...
        for_versions_to(2, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 45}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 28}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 17}, {cur4, 0}}},
                 {mm34, {{fno, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}}});
            // clang-format on
        });
        for_versions(3, 9, *app, [&] {
            ...
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance5 - 5 * txfee}, {cur1, 45}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 28}, {cur3, 0}, {cur4, 0}}},
                 {mm34, {{fno, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 17}, {cur4, 0}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment takes best offers, multiple offers per exchange")
    {
        ...
        for_versions(3, 9, *app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 16}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance5 - 5 * txfee}, {cur1, 29}, {cur2, 8}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 20}, {cur3, 4}, {cur4, 0}}},
                 {mm34, {{fno, minBalance5 - 5 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 13}, {cur4, 2}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 8}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment uses all offers in a loop")
    {
        ...
        for_all_versions(*app, [&] {
            ...
            // clang-format off
            market.requireBalances(
                {{source, {{fno, minBalance4 - 2 * txfee}, {cur1, 6720}, {cur2, 0}, {cur3, 0}, {cur4, 0}}},
                 {mm12, {{fno, minBalance3 - 3 * txfee}, {cur1, 1360}, {cur2, 7320}, {cur3, 0}, {cur4, 0}}},
                 {mm23, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 680}, {cur3, 7660}, {cur4, 0}}},
                 {mm34, {{fno, minBalance3 - 3 * txfee}, {cur1, 0}, {cur2, 0}, {cur3, 340}, {cur4, 7830}}},
                 {mm41, {{fno, minBalance3 - 3 * txfee}, {cur1, 7920}, {cur2, 0}, {cur3, 0}, {cur4, 160}}},
                 {destination, {{fno, minBalance1 - txfee}, {cur1, 0}, {cur2, 0}, {cur3, 0}, {cur4, 10}}}});
            // clang-format on
        });
    }
    ...
    SECTION("path payment with rounding errors")
    {
        ...
        auto sellerOffer = market.requireChangesWithOffer({}, [&] {
            return market.addOffer(seller, {cny, fno, price, 145000000});
        });
        ...
        for_versions_to(2, *app, [&] {
            ...
            REQUIRE_THROWS_AS(market.requireChanges(
                                  {},
                                  [&] {
                                      source.pay(destination, fno, 1382068965,
                                                 cny, 20000000, path);
                                  }),
                              ex_PATH_PAYMENT_TOO_FEW_OFFERS);
        });
        for_versions_from(3, *app, [&] {
            auto sellerOfferRemaining =
                OfferState{cny, fno, price, 145000000 - 20000000};
            auto actual = std::vector<ClaimOfferAtom>{};
            market.requireChanges(
                {{sellerOffer.key, sellerOfferRemaining}}, [&] {
                    actual = source
                                 .pay(destination, fno, 1382068965, cny,
            ...
            REQUIRE(actual == expected);
            market.requireBalances(
                {{source, {{fno, 1989999000 - 100 - 1379310345}}},
            ...
            "offers trail")
    {
        ...
        auto path = std::vector<Asset>{fno, usd};
        ...
        for_versions_to(2, *app, [&] {
            ...
            auto usdCurOffer = market.requireChangesWithOffer({}, [&] {
                return market.addOffer(mm,
                                       {usd, fno, Price{1, 2}, 2 * offerSize});
            });

            auto res = source.pay(destination, fno, 8 * paymentToReceive, idr,
    ...
    SECTION("path payment with cycle")
    {
        for_all_versions(*app, [&] {
            ...
            // buying price is
            // always half - so sender buys 8 times as much FNO
            // as he/she
            // sells (arbitrage).
            // Second cycle involves 3 transaction in which
            // buying price is
            // always two - so sender buys 8 times as much FNO
            // as he/she
            ...
            auto assets = std::deque<Asset>{fno, usd, idr};
    ...
    SECTION("liabilities")
    {
        SECTION("cannot pay balance below selling liabilities")
        {
            ...
            auto offer = market.requireChangesWithOffer({}, [&] {
                return market.addOffer(source, {cur1, fno, Price{1, 1}, 50});
            });
            auto o2 = market.requireChangesWithOffer({}, [&] {
