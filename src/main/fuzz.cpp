 ...
 *     random FoneroMessages. This is the mode you use to generate seed data
 *     for the external fuzzer's corpus.
 *
 *   - In --fuzz mode it reads back a file and appplies it to a pair of of
 *     fonero-cores in loopback mode, cranking the I/O loop to simulate
...
std::string
msgSummary(FoneroMessage const& m)
{
...
bool
tryRead(XDRInputFileStream& in, FoneroMessage& m)
{
    try
    {
        return in.readOne(m);
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Fuzzing fonero-core " << FONERO_CORE_VERSION;
...
genfuzz(std::string const& filename)
{
    ...
    out.open(filename);
    autocheck::generator<FoneroMessage> gen;
    for (size_t i = 0; i < n; ++i)
    {
        try
        {
            FoneroMessage m(gen(10));
            out.writeOne(m);
