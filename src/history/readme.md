...
The `get` and `put` commands define subprocesses that `fonero-core` will
...
called `s3fonero` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3fonero]
get="curl http://history.fonero.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.fonero.org/{1}"
~~~~

In this example, `fonero-core` will use the `curl` command to fetch history and
