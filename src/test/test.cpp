...
bool force_sqlite = (std::getenv("FONERO_FORCE_SQLITE") != nullptr);
...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("fonero-core-test");
        ...
        sstream << "fonero" << instanceNumber << ".log";
     ...
     std::vector<std::string> const& metrics)
{
    ...
    // not mixed with fonero-core logging.
    Logging::setFmt("<test>");
    ...
    LOG(INFO) << "Testing fonero-core " << FONERO_CORE_VERSION;
    ...
    cli |= clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "fonero-core can run tests concurrently");
    session.cli(cli);
