...
fonero-core can be controlled via the following commands.

## Command line options
* **--?** or **--help**: Print the available command line options and then exit..
* **--c** Send an [HTTP command](#http-commands) to an already running local instance of fonero-core and then exit. For example: 

`$ fonero-core -c info`

* **--conf FILE**: Specify a config file to use. You can use '-' and provide the config file via STDIN. *default 'fonero-core.cfg'*
...
`$ fonero-core --convertid SDQVDISRYN2JXBS7ICL7QJAEKB3HWBJFP2QECXG7GZICAHBK4UNJCWK2`
...
the database. The next time fonero-core is run, fonero-core will start 
emitting SCP messages based on its last known ledger. Without this flag fonero-core waits to hear
...
* **--newhist ARCH**:  Initialize the named history archive ARCH. ARCH should be one of the history archives you have specified in the fonero-core.cfg. This will write a `.well-known/fonero-history.json` file in the archive root.
...
* **--signtxn FILE**:  Add a digital signature to a transaction
  ...
  appears to be coming from a terminal, fonero-core disables echo.
  Note that if you do not have a FONERO_NETWORK_ID environment
...
* **--sec2pub**:  Reads a secret key on standard input and outputs the
  corresponding public key.  Both keys are in Fonero's standard
  base-32 ASCII format.
* **--netid STRING**:  The --signtxn option requires a particular
  network to sign for.  For example, the production fonero network is
  "`Public Global Fonero Network ; September 2015`" while the test
  network is "`Test SDF Network ; September 2015`".
* **--test**: Run all the unit tests.
  * Suboptions specific to fonero-core:
  ...
  * For example this will run just the tests tagged with `[tx]` using protocol
  ...
  `fonero-core --test -a --version 9 --version 10 "[tx]"`
* **--version**: Print version info and then exit.


## HTTP Commands
By default fonero-core listens for connections from localhost on port 11626. 
You can send commands to fonero-core via a web browser, curl, or using the --c 
...
* **setcursor**
 `/setcursor?id=ID&cursor=N`<br>
  ...
  Cursors are used by dependent services to tell fonero-core which data can be safely deleted by the instance.
...
* **testtx**
 `/testtx?from=F&to=T&amount=N&[create=true]`<br>
  Injects a payment transaction (or a create transaction if "create" is specified) from the account F to the account T, sending N FNO to the account.
